...
const closeWinModal = winModal.child('Close');
...
function animateFloating(obj, deltaY, ms){
	...
	// create sampler
	obj.animy_sampler = Animation.samplers.easeInOutSine(0, 0 + deltaY);
...
//TAP EVENTS
//TAP ON PIECES
TouchGestures.onTap(tapChunks[0]).subscribe(function (e) { animateScale(tapChunks[0], 1, 1000);  UIChunks[0].hidden = Reactive.val(true).delayBy({milliseconds: 1000}) ; fadeOut(flasher, mat_cloud, 400,0); fadeOut(uiflasher, mat_vaseUIFlash, 1000, 1000);});
TouchGestures.onTap(tapChunks[1]).subscribe(function (e) { animateScale(tapChunks[1], 1, 1000);  UIChunks[1].hidden = Reactive.val(true).delayBy({milliseconds: 1000}) ; fadeOut(flasher, mat_cloud, 400,0); fadeOut(uiflasher, mat_vaseUIFlash, 1000, 1000);});
TouchGestures.onTap(tapChunks[2]).subscribe(function (e) { animateScale(tapChunks[2], 1, 1000);  UIChunks[2].hidden = Reactive.val(true).delayBy({milliseconds: 1000}) ; fadeOut(flasher, mat_cloud, 400,0); fadeOut(uiflasher, mat_vaseUIFlash, 1000, 1000);});

//TAP ON VASE
TouchGestures.onTap(vase).subscribe(function () {	vase.hidden = true; 	spawnVase(); 	time = Reactive.val(-200); 	time = time.pin().add(Time.ms.sub(Time.ms.pin()).div(1000/8));	randomWinLose();	bindTime(); } );
